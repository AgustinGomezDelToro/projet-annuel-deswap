...
const Tokens = () => {
    ...
    async function fetchDatas() {
        try {
            const tokens: IToken[] = await new TokenService().getAll();
            setTokens(tokens);
        } catch (e) {
            console.error(e);
        }
    ...
    async function create() {
        try {
            ...
            const pools = (document.getElementById("pools") as HTMLInputElement).value;
            const logo = (document.getElementById("logo") as HTMLInputElement).files?.[0];
            if (!name || !symbol || !logo || isNaN(price)) {
                setMessage("Name, symbol, price, and logo are required");
            ...
            await new TokenService().create(name, symbol, price, address, deployedLogo.IpfsHash, pools, 0);
    ...
    async function update() {
        try {
            ...
            if (!name || !symbol || isNaN(price)) {
                setMessage("Name, symbol, and price are required");
                setNotification(true);
                return;
            }
            let deployedLogo = tokenSelected.logo;
            if (logo) {
                const logoUpload = await pinFileToIPFS(logo!);
                if (logoUpload) {
                    deployedLogo = logoUpload.IpfsHash;
                }
            }
            await new TokenService().update(tokenSelected.ID, name, symbol, price, address, deployedLogo, pools, tokenSelected.trades);
            ...
            console.error(e);
            setMessage("Error while updating token");
            setNotification(true);
        }
    }
    ...
    async function remove(address: string) {
        try {
            await new TokenService().delete(address);
            setTokenSelected(null);
        ...
        <section className="mx-auto max-w-7xl px-4 py-8 text-colors-black1">
            ...
            <div className="flex justify-center items-center">
                <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
                    <h2 className="text-2xl font-bold mb-4 text-center">{isEditing ? "Edit Token" : "Create New Token"}</h2>
                    <div className="space-y-4">
                        <input type="text" id="name" defaultValue={tokenSelected?.name || ""} placeholder="Name" className="w-full px-3 py-2 text-sm text-gray-700 placeholder-gray-500 bg-gray-100 border border-gray-400 rounded-lg focus:outline-none focus:ring-none" />
                        <input type="text" id="symbole" defaultValue={tokenSelected?.symbole || ""} placeholder="Symbol" className="w-full px-3 py-2 text-sm text-gray-700 placeholder-gray-500 bg-gray-100 border border-gray-400 rounded-lg focus:outline-none focus:ring-none" />
                        <input type="number" id="price" defaultValue={tokenSelected?.price?.toString() || ""} placeholder="Price" className="w-full px-3 py-2 text-sm text-gray-700 placeholder-gray-500 bg-gray-100 border border-gray-400 rounded-lg focus:outline-none focus:ring-none" />
                        <input type="text" id="address" defaultValue={tokenSelected?.address || ""} placeholder="Address (optional)" className="w-full px-3 py-2 text-sm text-gray-700 placeholder-gray-500 bg-gray-100 border border-gray-400 rounded-lg focus:outline-none focus:ring-none" />
                        <input type="text" id="pools" defaultValue={tokenSelected?.pools || ""} placeholder="Pools" className="w-full px-3 py-2 text-sm text-gray-700 placeholder-gray-500 bg-gray-100 border border-gray-400 rounded-lg focus:outline-none focus:ring-none" />
                        ...
                        <button disabled={notification} onClick={isEditing ? update : create} className="w-full py-2 text-white transition-colors duration-200 rounded-lg bg-blue-400 hover:bg-blue-500 focus:outline-none">
                                ...
                                {tokens && tokens.map((token, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className="px-4 py-4 space-x-2 text-sm text-right whitespace-nowrap">
                                            <button onClick={() => {setIsEditing(true); setTokenSelected(token);}} className="px-2 py-1 text-white transition-colors duration-200 rounded-lg bg-yellow-400 hover:bg-yellow-500 focus:outline-none">
                                                Edit
                                            </button>
                ...
                <div className="absolute top-2 left-1/2 -translate-x-1/2 rounded-2xl border border-blue-100 bg-white p-4 shadow-lg sm:p-6 lg:p-8" role="alert">
                    ...
                    <div className="mt-6 sm:flex sm:gap-4">
                        <button
                            ...
                            onClick={() => remove(tokenSelected?.address!)}
                        >
                            Validate
                        </button>
                        <button
